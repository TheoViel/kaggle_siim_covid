...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        loss = self.loss_fcn(pred, true)
        # p_t = torch.exp(-loss)
        # loss *= self.alpha * (1.000001 - p_t) ** self.gamma  # > 0 power for gradient stability
...
class ComputeLoss:
    """
    Wrapper to compute losses
    """
    def __init__(self, model, config):
        super(ComputeLoss, self).__init__()
        self.autobalance = config.autobalance
        self.config = config
        self.gr = config.gr  # wtf is that  (model.gr)

        device = next(model.parameters()).device  # get model device

        # Define criteria
        self.BCEcls = nn.BCEWithLogitsLoss(
            pos_weight=torch.tensor([config.cls_pw], device=device)
        )
        self.BCEobj = nn.BCEWithLogitsLoss(
            pos_weight=torch.tensor([config.obj_pw], device=device)
        )

        # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
        self.cp, self.cn = smooth_BCE(
            eps=config.label_smoothing  # should be 0
        )  # positive, negative BCE targets

        # Focal loss
        if config.fl_gamma > 0:
            self.BCEcls = FocalLoss(self.BCEcls, config.fl_gamma),
            self.BCEobj = FocalLoss(self.BCEobj, config.fl_gamma)
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        lbox *= self.config.box
        lobj *= self.config.obj
        lcls *= self.config.cls
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                j = torch.max(r, 1.0 / r).max(2)[0] < self.config.anchor_t  # compare
                t = t[j]  # filter
