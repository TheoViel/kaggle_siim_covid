...
def generate_detections(
    ...
    classes = (
        classes[top_detection_idx, None] + 1
    )  # back to class idx with background class = 0
...
class Anchors(nn.Module):
    ...
    def __init__(
        ...
        assert (
            image_size[0] % 2 ** max_level == 0
        ), "Image size must be divisible by 2 ** max_level (128)"
        assert (
            image_size[1] % 2 ** max_level == 0
        ), "Image size must be divisible by 2 ** max_level (128)"
    ...
    def label_anchors(self, gt_boxes, gt_classes, filter_valid=True):
        ...
        Returns:
            cls_targets_dict: ordered dictionary with keys [min_level, min_level+1, ..., max_level].
                The values are tensor with shape [height_l, width_l, num_anchors].
            ...
            box_targets_dict: ordered dictionary with keys [min_level, min_level+1, ..., max_level].
                The values are tensor with shape [height_l, width_l, num_anchors * 4].
                The height_l and width_l represent the dimension of
                bounding box regression output at l-th level.
        ...
        for level in range(self.anchors.min_level, self.anchors.max_level + 1):
            ...
            steps = (
                feat_size[0] * feat_size[1] * self.anchors.get_anchors_per_location()
            )
        ...
        for i in range(batch_size):
            ...
            for level in range(self.anchors.min_level, self.anchors.max_level + 1):
                ...
                steps = (
                    feat_size[0]
                    * feat_size[1]
                    * self.anchors.get_anchors_per_location()
                )
