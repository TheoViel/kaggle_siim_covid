...
def _post_process(
    ...
    Post-proc code adapted from Tensorflow version at:
...
class DetBenchTrain(nn.Module):
    ...
    def forward(self, x, target: Dict[str, torch.Tensor]):
        ...
        if self.anchor_labeler is None:
            ...
            cls_targets = [target[f"label_cls_{lvl}"] for lvl in range(self.num_levels)]
            box_targets = [target[f"label_bbox_{lvl}"] for lvl in range(self.num_levels)]
...
def unwrap_bench(model):
    # Unwrap a model in support bench so that various other fns can access the weights and
    # attribs of the underlying model directly
